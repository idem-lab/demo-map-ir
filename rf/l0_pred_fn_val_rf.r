pred_val_rf <- function(tune_run, tune_run2, val_run) {
  ...
  stk_val_inds_i <-
    c(stk_val_inds[[val_run]][[1]],
      stk_val_inds[[val_run]][[2]],
      stk_val_inds[[val_run]][[3]],
      stk_val_inds[[val_run]][[4]])

  # ==== logit transform on pct mortality
  #Empirical logit and IHS transform on labels
  data_all[, "pcent_mortality"] <-
    emplogit2(data_all["no_dead"], data_all["no_tested"])
  theta2 <-
    optimise(
      IHS.loglik,
      lower = 0.001,
  ...
  data_all[, "pcent_mortality"] <-
    IHS(data_all[, "pcent_mortality"], theta2$maximum)
  ...
  test_inds <-
    test_inds_a[which(!is.element(test_inds_a, stk_val_inds_i))]

  testJ <- data_all[test_inds, ]

  train_inds <-
    which(!is.element(1:nrow(data_all), c(val_ind[[1]], stk_val_inds_i)))
  ...
  rfFitJ <-
    train(
      ...
      replace = TRUE,
      metric = "RMSE",
      col_sample_rate_per_tree = paramsTuneGrid$col_sample_rate_per_tree,
  ...
  for (i in 2:length(stk_val_inds)) {
    ...
    testJ <- data_all[test_inds, ]
    train_inds <-
      which(!is.element(1:nrow(data_all), c(val_ind[[i]], stk_val_inds_i)))
    trainJ <- data_all[train_inds, ]
