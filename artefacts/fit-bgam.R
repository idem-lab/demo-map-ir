fit_bgam <- function(data_all,
  ...
  data_all[, "pcent_mortality"] <- emplogit2(data_all["no_dead"],
                                             data_all["no_tested"])
  ...
  data_all[, "pcent_mortality"] <- IHS(data_all[, "pcent_mortality"],
                                       theta2$maximum)
  ...
  outer_test_index <-
    unlist(outer_test_index_list[[outer_validation]])

  test_inds <-
    test_inds_a[which(!is.element(test_inds_a, outer_test_index))]
  test_data <-
    data_all[test_inds, c("pcent_mortality", covariate_names, "int")]
  train_data <-
    data_all[-test_inds, c("pcent_mortality", covariate_names, "int")]
  ...
  cts_cov_inds <-
    which(!is.element(covariate_names, covariate_names_a))
  ...
  covariate_names_b <-
    c("bols(int,intercept=FALSE)", covariate_names_b)
  ...
  gamb_fit_train <- gamboost(
    form,
    data = train_data,
    control = boost_control(mstop = gamb_tune_grid,
                            nu = 0.4,
                            risk = "oobag")
  )
  ...
  # ==== calculate rmse ====
  errors <-
    gamb_predict_test - data_all[test_inds, "pcent_mortality"]
